...
namespace DoozyUI
{
    ...
    public class UIEffect : MonoBehaviour
    {
        ...
        /// </summary>
        private static Dictionary<string, List<UIEffect>> m_Database;
        /// <summary>
        /// Gets a registry of all the registered UIEffects.
        /// </summary>
        /// <Value>
        /// The database.
        /// </Value>
        public static Dictionary<string, List<UIEffect>> Database { get { if(m_Database == null) { m_Database = new Dictionary<string, List<UIEffect>>(); } return m_Database; } }
        ...
        {
            List<UIEffect> tempUIEffectsList = new List<UIEffect>();
            if(Database != null && Database.Count > 0 && Database.ContainsKey(elementName))
            {
                int count = Database[elementName].Count;
                for(int i = 0; i < count; i++)
                {
                   if(Database[elementName][i].targetUIElement == null) { continue;}
                   if(Database[elementName][i].targetUIElement.elementCategory != elementCategory) { continue; }
                    tempUIEffectsList.Add(Database[elementName][i]);
                }
            }
            return tempUIEffectsList;
        }
        #endregion
    }
}
