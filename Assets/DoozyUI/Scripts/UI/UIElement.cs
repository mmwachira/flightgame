...
namespace DoozyUI
{
    ...
    public class UIElement : MonoBehaviour
    {
        ...
        /// </summary>
        private static Dictionary<string, List<UIElement>> m_Database;
        /// <summary>
        /// Gets a registry of all the registered UIElements.
        /// </summary>
        /// <Value>
        /// The database.
        /// </Value>
        public static Dictionary<string, List<UIElement>> Database { get { if(m_Database == null) { m_Database = new Dictionary<string, List<UIElement>>(); } return m_Database; } }
        ...
        void Start()
        {
            if(autoRegister) { RegisterUIElement(); }
        ...
        /// <summary>
        /// Registers this UIElement to the UIElements Database.
        /// </summary>
        public void RegisterUIElement()
        {
            ...
            if(Database.ContainsKey(elementName))
            {
                if(Database[elementName] == null)
                {
                    Database[elementName] = new List<UIElement>();
                }
                if(Database[elementName].Contains(this))
                {
                    return;
                }
                Database[elementName].Add(this);
            }
            else
            {
                Database.Add(elementName, new List<UIElement>() { this });
            }
        }
        /// <summary>
        ...
        public void UnregisterUIElement()
        {
            ...
            if(Database.ContainsKey(elementName))
            {
                Database[elementName].Remove(this);
                if(Database[elementName].Count == 0)
                {
                    Database.Remove(elementName);
                }
            }
        }
        #endregion
        ...
        public void InitLoopAnimations(bool forced = false)
        {
        ...
        /// </summary>
        public void PlayLoopAnimations()
        {
            UIAnimator.SetupLoops(RectTransform, startPosition, startRotation, startScale, startAlpha, loopAnimations,
                    null, null,
                    null, null,
        ...
        /// <summary>
        /// Gets a list of all UIElements that have the given name and category. If no UIElement was found, it will return an empty list.
        /// </summary>
        ...
        {
            List<UIElement> tempUIElementsList = new List<UIElement>();
            if(Database != null && Database.Count > 0 && Database.ContainsKey(elementName))
            {
                int count = Database[elementName].Count;
                for(int i = 0; i < count; i++)
                {
                    if(Database[elementName][i].elementCategory != elementCategory) { continue; }
                    tempUIElementsList.Add(Database[elementName][i]);
                }
            }
            return tempUIElementsList;
        }

        /// <summary>
        /// Gets a list of all the UIElements that are visible on the screen. An UIElement is considered visible if its isVisible Value is true.
        /// </summary>
        /// <returns></returns>
        public static List<UIElement> GetVisibleUIElements()
        {
            List<UIElement> tempUIElementsList = new List<UIElement>();
            foreach(KeyValuePair<string, List<UIElement>> kvp in Database)
            {
                ...
                for(int i = 0; i < count; i++)
                {
                    ...
                    tempUIElementsList.Add(kvp.Value[i]);
                }
            }
            return tempUIElementsList;
        }
        ...
        /// </returns>
        public static bool IsUIElementVisible(string elementName, string elementCategory = DUI.UNCATEGORIZED_CATEGORY_NAME)
        {
            List<UIElement> tempList = new List<UIElement>();
            tempList = GetVisibleUIElements();
            for(int i = 0; i < tempList.Count; i++)
            {
                if(tempList[i].elementName.Equals(elementName) && tempList[i].elementCategory.Equals(elementCategory))
                {
                    return true;
                }
            }
        ...
        /// </summary>
        public static void ShowUIElement(string elementName, string elementCategory = DUI.UNCATEGORIZED_CATEGORY_NAME) { ShowUIElement(elementName, elementCategory, false); }
        /// <summary>
        ...
        public static void ShowUIElement(string elementName, bool instantAction) { ShowUIElement(elementName, DUI.UNCATEGORIZED_CATEGORY_NAME, false); }
        /// <summary>
        /// Shows all the UIElements that have the given name and category.
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void ShowUIElement(string elementName, string elementCategory, bool instantAction)
        {
            ...
            ExecuteShow(elementName, elementCategory, instantAction);
        }
        ...
        /// </summary>
        private static void ExecuteShow(string elementName, string elementCategory, bool instantAction)
        {
            List<UIElement> tempUIElementsList = GetUIElements(elementName, elementCategory);
            int elementsCount = tempUIElementsList != null ? tempUIElementsList.Count : 0;
            if(elementsCount > 0)
            {
                for(int i = 0; i < elementsCount; i++)
                {
                    if(tempUIElementsList[i] != null) //this null check has been added to fix the slim chance that we registered a UIElement to the registry and it has been destroyed/deleted (thus now it's null)
                    {
                        tempUIElementsList[i].gameObject.SetActive(true);
                        if(tempUIElementsList[i].gameObject.activeInHierarchy)
                        {
                            if(!UIManager.useOrientationManager)
                            {
                                if(UIManager.Instance.debugUIElements) { Debug.Log("[DoozyUI] Showing the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                                tempUIElementsList[i].Show(instantAction);
                            }
                            else
                            {
                                if(UIManager.Instance.currentOrientation == UIManager.Orientation.Unknown)
                                ...
                                if(UIManager.Instance.currentOrientation == UIManager.Orientation.Landscape)
                                {
                                    if(tempUIElementsList[i].LANDSCAPE)
                                    {
                                        if(UIManager.Instance.debugUIElements) { Debug.Log("[DoozyUI] Showing the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                                        tempUIElementsList[i].Show(instantAction);
                                    }
                                    else if(tempUIElementsList[i].PORTRAIT)
                                    {
                                        tempUIElementsList[i].isVisible = true;
                                        tempUIElementsList[i].Hide(true);
                                    }
                                    else
                                    {
                                        tempUIElementsList[i].isVisible = true;
                                ...
                                else if(UIManager.Instance.currentOrientation == UIManager.Orientation.Portrait)
                                {
                                    if(tempUIElementsList[i].PORTRAIT)
                                    {
                                        if(UIManager.Instance.debugUIElements) { Debug.Log("[DoozyUI] Showing the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                                        tempUIElementsList[i].Show(instantAction);
                                    }
                                    else if(tempUIElementsList[i].LANDSCAPE)
                                    {
                                        tempUIElementsList[i].isVisible = true;
                                        tempUIElementsList[i].Hide(true);
                                    }
            ...
            List<UIEffect> tempUIEffectsList = UIEffect.GetUIEffects(elementName, elementCategory);
            int effectsCount = tempUIEffectsList != null ? tempUIEffectsList.Count : 0;
            if(effectsCount > 0)
            {
                for(int i = 0; i < effectsCount; i++)
                {
                    tempUIEffectsList[i].gameObject.SetActive(true);
                    if(tempUIEffectsList[i].gameObject.activeInHierarchy)
                    {
                        if(UIManager.useOrientationManager == false)
                        {
                            tempUIEffectsList[i].Show();
                        }
                        else
                        {
                            if(UIManager.Instance.currentOrientation == UIManager.Orientation.Landscape)
                            {
                                if(tempUIEffectsList[i].targetUIElement.LANDSCAPE)
                                {
                                    tempUIEffectsList[i].Show();
                                }
                                else if(tempUIEffectsList[i].targetUIElement.PORTRAIT)
                                {
                                    tempUIEffectsList[i].isVisible = true;
                                    tempUIEffectsList[i].Hide();
                                }
                                else
                                {
                                    tempUIEffectsList[i].isVisible = true;
                            ...
                            {
                                if(tempUIEffectsList[i].targetUIElement.PORTRAIT)
                                {
                                    tempUIEffectsList[i].Show();
                                }
                                else if(tempUIEffectsList[i].targetUIElement.LANDSCAPE)
                                {
                                    tempUIEffectsList[i].isVisible = true;
                                    tempUIEffectsList[i].Hide();
                                }
        ...
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void HideUIElement(string elementName, string elementCategory) { HideUIElement(elementName, elementCategory, false); }
        /// <summary>
        /// Hides all the UIElements that have the given name and the DEFAULT CATEGORY name.
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        public static void HideUIElement(string elementName, bool instantAction = false) { HideUIElement(elementName, DUI.UNCATEGORIZED_CATEGORY_NAME, instantAction); }
        /// <summary>
        /// Hides all the UIElements that have the given name and category.
        /// </summary>
        /// <param name="instantAction">Should the animation play instantly (in zero seconds)</param>
        ...
        {
            if(elementName.Equals(DUI.DEFAULT_ELEMENT_NAME)) { return; }
            ExecuteHide(elementName, elementCategory, instantAction);
        }

        /// <summary>
        /// This IEnumerator executes the Hide actions and forces an Instance if required.
        /// </summary>
        private static void ExecuteHide(string elementName, string elementCategory, bool instantAction)
        {
            List<UIElement> tempUIElementsList = GetUIElements(elementName, elementCategory);
            int elementsCount = tempUIElementsList != null ? tempUIElementsList.Count : 0;
            if(elementsCount > 0)
            {
                for(int i = 0; i < elementsCount; i++)
                {
                    if(tempUIElementsList[i] != null) //this null check has been added to fix the slim chance that we registered a UIElement to the registry and it has been destroyed/deleted (thus now it's null)
                    {
                        if(tempUIElementsList[i].gameObject.activeInHierarchy)
                        {
                            if(UIManager.Instance.debugUIElements) { Debug.Log("[DoozyUI] Hiding the (" + elementCategory + ")(" + elementName + ") UIElement. This is " + (instantAction ? "" : "not ") + "an instant action."); }
                            tempUIElementsList[i].Hide(instantAction);
                        }
                    }
                }
            }
            ...
            List<UIEffect> tempEffectsList = UIEffect.GetUIEffects(elementName, elementCategory);
            int effectsCount = tempEffectsList != null ? tempEffectsList.Count : 0;
            if(effectsCount > 0)
            {
                for(int i = 0; i < effectsCount; i++)
                {
                    if(tempEffectsList[i].gameObject.activeInHierarchy)
                    {
                        tempEffectsList[i].Hide();
                    }
                }
            }
        }
