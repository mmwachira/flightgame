...
namespace DoozyUI
{
    ...
    public class UIButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler, IPointerClickHandler, ISelectHandler, IDeselectHandler
    {
        ...
        void ExecuteOnClickActions()
        {
            ...
            if(!useOnClickAnimations) //check if the OnClick is disabled
            {
                if(!useOnDoubleClick && !useOnLongClick) //if don't use OnDobuleClick and OnLongClick then return
                {
                    return;
                }

                if(useOnLongClick && !useOnClickAnimations) //if OnLongClick is enabled, but OnClick is not then return (as we do not want OnClick to trigger)
                {
                    return;
                }
            }
