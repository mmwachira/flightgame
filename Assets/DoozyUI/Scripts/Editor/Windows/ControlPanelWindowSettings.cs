    ...
    public partial class ControlPanelWindow : QWindow
    {
        ...
        void DrawPageEditorSettings()
        {
            ...
            QUI.BeginChangeCheck();
            {
                ...
                DrawGeneralSettings(sectionWidth);
                QUI.Space(SPACE_16);
        ...
        void DrawGeneralSettings(float width)
        {
            QLabel.text = "General Settings";
            QLabel.style = Style.Text.Subtitle;
            QUI.Label(QLabel);

            EditorSettings.autoExpandEnabledFeatures = QUI.QToggle("[General] Auto Expand Enabled Features - when selecting a DoozyUI component", EditorSettings.autoExpandEnabledFeatures);
            EditorSettings.askToAddElementNameToCategoryIfNotFound = QUI.QToggle("[UIElement] Ask to add Element Name to newly selected Cateogry - if the name was not found", EditorSettings.askToAddElementNameToCategoryIfNotFound);
            EditorSettings.askToAddButtonNameToCategoryIfNotFound = QUI.QToggle("[UIButton] Ask to add Button Name to newly selected Cateogry - if the name was not found", EditorSettings.askToAddButtonNameToCategoryIfNotFound);
        }
                ...
                if(QUI.BeginFadeGroup(UIElementAnimBool.faded))
                {
                    QUI.BeginVertical(tempWidth);
                    {
                        ...
                        QUI.BeginHorizontal(tempWidth);
                        {
                            ...
                            QLabel.style = Style.Text.Small;
                            tempFloat = QLabel.x + 120 + 8;
                            QUI.Box(QStyles.GetBackgroundStyle(Style.BackgroundType.Low, EditorSettings.UIElement_loadLoopAnimationsPresetAtRuntime ? QColors.Color.Blue : QColors.Color.Gray), tempFloat + 16, 20);
                            QUI.Space(-tempFloat - 12);
                            QUI.BeginVertical(QLabel.x, 20);
                            {
                                QUI.Space((20 - 16) / 2);
                            ...
                            QUI.EndVertical();
                            EditorSettings.UIElement_loopAnimationsPresetCategoryName = EditorGUILayout.DelayedTextField(EditorSettings.UIElement_loopAnimationsPresetCategoryName, GUILayout.Width(120));
                            ...
                            QUI.EndVertical();
                            EditorSettings.UIElement_loopAnimationsPresetName = EditorGUILayout.DelayedTextField(EditorSettings.UIElement_loopAnimationsPresetName, GUILayout.Width(120));

                            QUI.Space(SPACE_4);
                            QUI.Space(SPACE_4 * UIElementAnimBool.faded);

                            EditorSettings.UIElement_loadLoopAnimationsPresetAtRuntime = QUI.QToggle("Load preset at runtime", EditorSettings.UIElement_loadLoopAnimationsPresetAtRuntime, 20, Style.Text.Small);
