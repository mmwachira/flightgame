...
#pragma warning restore 0414
        ...
        protected override void InitAnimBools()
        {
            ...
            showInAnimations = new AnimBool(EditorSettings.autoExpandEnabledFeatures ? uiElement.InAnimationsEnabled || loadInAnimationsPresetAtRuntime.boolValue : false, Repaint);
            showInAnimationsEvents = new AnimBool(EditorSettings.autoExpandEnabledFeatures ? uiElement.OnInAnimationsStart.GetPersistentEventCount() > 0 || uiElement.OnInAnimationsFinish.GetPersistentEventCount() > 0 : false, Repaint);
            showInAnimationsPreset = new AnimBool(loadInAnimationsPresetAtRuntime.boolValue, Repaint);

            showOutAnimations = new AnimBool(EditorSettings.autoExpandEnabledFeatures ? uiElement.OutAnimationsEnabled || loadOutAnimationsPresetAtRuntime.boolValue : false, Repaint);
            showOutAnimationsEvents = new AnimBool(EditorSettings.autoExpandEnabledFeatures ? uiElement.OnOutAnimationsStart.GetPersistentEventCount() > 0 || uiElement.OnOutAnimationsFinish.GetPersistentEventCount() > 0 : false, Repaint);
            showOutAnimationsPreset = new AnimBool(loadOutAnimationsPresetAtRuntime.boolValue, Repaint);

            showLoopAnimations = new AnimBool(EditorSettings.autoExpandEnabledFeatures ? uiElement.LoopAnimationsEnabled || loadLoopAnimationsPresetAtRuntime.boolValue : false, Repaint);
        ...
        protected override void OnEnable()
        {
            ...
            currentCategoryNameIsCustomName = elementCategory.stringValue.Equals(DUI.CUSTOM_NAME); //note that the previous category name is Custom; we need this in order to ask the dev if he wants to add a new element name to a newly selected category
        }
                ...
                else
                {
                    ...
                    if(QUI.EndChangeCheck())
                    {
                        ...
                        if(elementCategory.stringValue == DUI.CUSTOM_NAME)
                        {
                            ...
                            currentCategoryNameIsCustomName = true; //note that the previous category name is Custom; we need this in order to ask the dev if he wants to add a new element name to a newly selected category
                        }
                        serializedObject.ApplyModifiedProperties();
                    }
                }
            ...
            QUI.BeginHorizontal(width);
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            LockEditor();
                            if((EditorSettings.askToAddElementNameToCategoryIfNotFound || currentCategoryNameIsCustomName) &&
                        ...
                        currentCategoryNameIsCustomName = false; //note that the previous category name is not Custom; we need this in order to ask the dev if he wants to add a new element name to a newly selected category
                    }
                }
        ...
        void DrawSettings(float width)
        {
            ...
            QUI.BeginHorizontal(width);
            {
                ...
                QLabel.style = Style.Text.Normal;
                QUI.Box(QStyles.GetBackgroundStyle(Style.BackgroundType.Low, autoHide.boolValue ? QColors.Color.Blue : QColors.Color.Gray), QLabel.x + 16 + 16 + 174 * showAutoHideDelay.faded, 19);
                QUI.Space(-QLabel.x - 12 - 14 - 174 * showAutoHideDelay.faded);

                QUI.BeginVertical(13);
                {
                    QUI.Toggle(autoHide);
                }
                QUI.EndVertical();
            ...
            QUI.BeginHorizontal(width);
            {
                ...
                QUI.Box(QStyles.GetBackgroundStyle(Style.BackgroundType.Low, useCustomStartAnchoredPosition.boolValue ? QColors.Color.Blue : QColors.Color.Gray), QLabel.x + 16 + 16 + tempFloat * showCustomStartPosition.faded, 18 + 24 * showCustomStartPosition.faded);
                QUI.Space(-QLabel.x - 12 - 16 - tempFloat * showCustomStartPosition.faded);

                QUI.Space(2);
                QUI.BeginVertical(12);
                {
            ...
            QUI.BeginHorizontal(width);
            {
                ...
                QUI.Box(QStyles.GetBackgroundStyle(Style.BackgroundType.Low, executeLayoutFix.boolValue ? QColors.Color.Blue : QColors.Color.Gray), tempFloat + 16 + 20, 18); //draw background by adding the Toggle width and the extra spaces
                QUI.Space(-tempFloat - 14 - 16);
