...
namespace DoozyUI
{
    public class DUIUtils
    {
        ...
        public static void DrawNavigation(QEditor editor, DUIComponent component, ButtonEventType buttonEventType, NavigationPointerData navigationPointerData, EditorNavigationPointerData editorNavigationPointerData, AnimBool show, UnityAction UpdateAllNavigationData, bool useNavigationHistory, bool isBackButton, float width, float miniBarHeight)
        {
            ...
            QUI.BeginHorizontal(width);
            {
                ...
                if(QUI.BeginFadeGroup(show.faded))
                {
                    ...
                    {
                        QUI.Space(2);
                        DrawNavigationData(editor, component, buttonEventType, navigationPointerData, editorNavigationPointerData, UpdateAllNavigationData, useNavigationHistory, isBackButton, width - 8);
                        QUI.Space(8);
                    }
                    QUI.EndVertical();
                }
        ...
        private static void DrawNavigationData(QEditor editor, DUIComponent component, ButtonEventType buttonEventType, NavigationPointerData navData, EditorNavigationPointerData editorNavData, UnityAction UpdateAllNavigationData, bool useNavigationHistory, bool isBackButton, float width)
        {
            if(useNavigationHistory)
            {
                QUI.BeginHorizontal(width);
                {
                    ...
                    if(QUI.EndChangeCheck())
                    {
                        ...
                        {
                            Undo.RecordObjects(editor.targets, "ToggleNavHistory");
                            for(int i = 0; i < editor.targets.Length; i++)
                            {
                                GetTargetNavigationPointerData(editor.targets[i], component, buttonEventType).addToNavigationHistory = tempBool;
                            }
                        }
                        else
                        {
            ...
            DrawNavigationDataList(editor, component, buttonEventType, NavigationType.Show, navData.show, editorNavData.showIndex, UpdateAllNavigationData, useNavigationHistory, isBackButton, width);
            QUI.Space(4);
            DrawNavigationDataList(editor, component, buttonEventType, NavigationType.Hide, navData.hide, editorNavData.hideIndex, UpdateAllNavigationData, useNavigationHistory, isBackButton, width);
        }
        private static void DrawNavigationDataList(QEditor editor, DUIComponent component, ButtonEventType buttonEventType, NavigationType navigationType, List<NavigationPointer> list, List<EditorNavigationPointer> listIndex, UnityAction UpdateAllNavigationData, bool useNavigationHistory, bool isBackButton, float width)
        {
                        ...
                        for(int i = 0; i < list.Count; i++)
                        {
                            QUI.BeginHorizontal(width);
                            {
                                ...
                                if(QUI.EndChangeCheck())
                                {
                                    if(editor.serializedObject.isEditingMultipleObjects)
                                    {
                                        Undo.RecordObjects(editor.targets, "UpdateNavigation");
                                        list[i].category = DUIData.Instance.DatabaseUIElements.categoryNames[listIndex[i].categoryIndex];
                                        if(list[i].category.Equals(DUI.CUSTOM_NAME)) { listIndex[i].nameIndex = -1; }

                                        for(int j = 0; j < editor.targets.Length; j++)
                                        {
                                            switch(navigationType)
                                            {
                                                case NavigationType.Show: GetTargetNavigationPointerData(editor.targets[j], component, buttonEventType).show = new List<NavigationPointer>(list); break;
                                                case NavigationType.Hide: GetTargetNavigationPointerData(editor.targets[j], component, buttonEventType).hide = new List<NavigationPointer>(list); break;
                                            }
                                        }
                                    }
                                    else
                                ...
                                QUI.BeginVertical(16, 16);
                                {
                                    if(QUI.ButtonMinus())
                                    {
                                        if(editor.serializedObject.isEditingMultipleObjects)
                                        {
                                            Undo.RecordObjects(editor.targets, "Removed NavigationPointer");
                                            list.RemoveAt(i);
                        ...
                        QUI.BeginHorizontal(width);
                        {
                            ...
                            QUI.BeginVertical(16, 16);
                            {
                                if(QUI.ButtonPlus())
                                {
                                    if(editor.serializedObject.isEditingMultipleObjects)
                                    {
                                        Undo.RecordObjects(editor.targets, "Added NavigationPointer");
        ...
        private static NavigationPointerData GetTargetNavigationPointerData(Object target, DUIComponent component, ButtonEventType buttonEventType)
        {
            switch(component)
            {
                case DUIComponent.UIButton:
                    UIButton button = (UIButton)target;
                    switch(buttonEventType)
                    {
                        case ButtonEventType.OnPointerEnter: return button.onPointerEnterNavigation;
                        case ButtonEventType.OnPointerExit: return button.onPointerExitNavigation;
                        case ButtonEventType.OnPointerDown: return button.onPointerDownNavigation;
                        case ButtonEventType.OnPointerUp: return button.onPointerUpNavigation;
                        case ButtonEventType.OnClick: return button.onClickNavigation;
                        case ButtonEventType.OnDoubleClick: return button.onDoubleClickNavigation;
                        case ButtonEventType.OnLongClick: return button.onLongClickNavigation;
                    }
                    break;
                case DUIComponent.UIToggleOn:
                    UIToggle toggleOn = (UIToggle)target;
                    switch(buttonEventType)
                    {
                        case ButtonEventType.OnPointerEnter: return toggleOn.onPointerEnterNavigationToggleOn;
                        case ButtonEventType.OnPointerExit: return toggleOn.onPointerExitNavigationToggleOn;
                        case ButtonEventType.OnClick: return toggleOn.onClickNavigationToggleOn;
                    }
                    break;
                case DUIComponent.UIToggleOff:
                    UIToggle toggleOff = (UIToggle)target;
                    switch(buttonEventType)
                    {
                        case ButtonEventType.OnPointerEnter: return toggleOff.onPointerEnterNavigationToggleOff;
                        case ButtonEventType.OnPointerExit: return toggleOff.onPointerExitNavigationToggleOff;
                        case ButtonEventType.OnClick: return toggleOff.onClickNavigationToggleOff;
                    }
                    break;
#if dUI_DoozyUI
                case DUIComponent.GestureDetector:
                    Gestures.GestureDetector gestureDetector = (Gestures.GestureDetector)target;
                    return gestureDetector.navigationPointerData;
#endif
            }
